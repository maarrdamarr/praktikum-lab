        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            ...
            <div class="p-10 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[3rem] shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ auth()->user()->name }}" class="w-full bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Institusi</label>
                        <input type="email" value="{{ auth()->user()->email }}" disabled class="w-full bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl px-6 py-4 text-sm font-bold text-slate-400 cursor-not-allowed">
                    </div>
                    <div class="space-y-2">
            ...
            <div class="p-10 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[3rem] shadow-sm">
                ...
                <div class="p-12 border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-3xl text-center bg-gray-50/50 dark:bg-slate-900/50 hover:border-blue-500 transition-all group relative">
                    <input type="file" name="identity" id="identity_file" class="hidden" {{ auth()->user()->identity_path ? '' : 'required' }}>
