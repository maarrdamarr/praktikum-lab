    ...
    <div class="space-y-6">
        @if($moduls->isEmpty())
            <div class="p-16 text-center bg-white dark:bg-slate-900 border-[3px] border-slate-900 dark:border-white rounded-2xl shadow-[6px_6px_0px_#000] dark:shadow-[6px_6px_0px_#fff]">
                <div class="w-20 h-20 mx-auto mb-4 bg-amber-100 dark:bg-amber-900/30 border-[3px] border-slate-900 dark:border-white rounded-2xl flex items-center justify-center">
                    <svg class="w-10 h-10 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </div>
                <p class="text-sm font-black uppercase tracking-widest text-slate-400">Belum ada modul yang tersedia untuk Anda saat ini.</p>
            </div>
        @else
            {{-- Grid card view for dosen --}}
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                @foreach($moduls as $modul)
                    ...
                    <div class="bg-white dark:bg-slate-900 border-[3px] border-slate-900 dark:border-white rounded-2xl shadow-[4px_4px_0px_#000] dark:shadow-[4px_4px_0px_#fff] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_#000] dark:hover:shadow-[6px_6px_0px_#fff] transition-all overflow-hidden flex flex-col">
                        ...
                        {{-- Amber top bar for dosen --}}
                        <div class="h-2 bg-amber-400"></div>

                        <div class="p-6 flex-1 flex flex-col">
                            <div class="flex items-start justify-between mb-4">
                                <div>
                                    <span class="px-3 py-1.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-[2px] border-slate-900 dark:border-white text-[9px] font-black uppercase tracking-widest rounded-lg shadow-[2px_2px_0px_#000] dark:shadow-[2px_2px_0px_#fff] block w-fit mb-2">
                                        {{ $modul->nomor_modul }}
                                    </span>
                                    <h3 class="text-base font-black text-slate-900 dark:text-white leading-tight">{{ $modul->judul }}</h3>
                                    @if($modul->mata_kuliah)
                                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">📚 {{ $modul->mata_kuliah }}</p>
                                    @endif
                                </div>
                                <div class="w-14 h-14 {{ $color }} border-[3px] border-slate-900 dark:border-white rounded-xl flex items-center justify-center text-white font-black text-[11px] shadow-[3px_3px_0px_#000] dark:shadow-[3px_3px_0px_#fff] shrink-0 ml-3">
                                    {{ $label }}
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between mt-4 pt-4 border-t-[2px] border-slate-200 dark:border-slate-700">
                                <div class="space-y-0.5">
                                    <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest">Versi {{ $modul->versi }}</span>
                                    @if($modul->file_size)
                                        <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ $modul->file_size_format }}</span>
                                    @endif
                                </div>
                                @if($modul->file_path)
                                    <a href="{{ route('dosen.modul.download', $modul->id) }}"
                                       class="flex items-center gap-2 px-5 py-2.5 bg-amber-400 text-slate-900 border-[2px] border-slate-900 dark:border-white text-[9px] font-black uppercase tracking-widest rounded-xl shadow-[2px_2px_0px_#000] dark:shadow-[2px_2px_0px_#fff] hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-[3px_3px_0px_#000] transition-all">
